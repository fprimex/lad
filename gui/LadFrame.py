...
from gui import GraphCanvas
# end wxGlade

from . import NotebookPage
from . import ExportAsImageDlg
from . import EditAxesDlg
from . import EditSelectionDlg
...
class LadFrame(wx.Frame):
  ...
  def __init__(self, *args, **kwds):
    ...
    self.canvas = GraphCanvas(self.left_pane, wx.ID_ANY)
  ...
  def InitShell(self):
    ...
    app      The wxPython application object (wx.App)
    \"\"\")
    """)
  ...
  def ReinitShell(self):
    self.shell.push("import networkx as nx", silent=True)
    self.shell.push("app = wx.GetApp()", silent=True)
    self.shell.push("canvas = app.canvas", silent=True)
    self.shell.push("G = canvas.G", silent=True)
    self.shell.push("notebook = app.notebook", silent=True)
    self.shell.push("shell = app.shell", silent=True)
  ...
  def OnOpen(self, event): # wxGlade: LadFrame.<event_handler>
    ...
    if dlg.ShowModal() == wx.ID_OK:
      ...
      open_dlg = wx.FileDialog(self, "Open file...", "", "", "LAD files (*.lad)|*.lad")
      if open_dlg.ShowModal() == wx.ID_OK:
        graph_filename = open_dlg.GetFilename()
        graph_full_filename = open_dlg.GetPath()
        file = open(graph_full_filename, 'rb')
        print(graph_full_filename)
        self.canvas.G = pickle.load(file)
        file.close()
        self.ReinitNotebook()
        self.ReinitShell()
        self.canvas.Refresh()
  ...
  def OnSaveAs(self, event): # wxGlade: LadFrame.<event_handler>
    ...
    if save_dlg.ShowModal() == wx.ID_OK:
      ...
      try:
        ...
        pickle.dump(self.canvas.G, file)
        file.close()
